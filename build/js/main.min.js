...
var i,acc=document.getElementsByClassName("catalog-filters");for(i=0;i<acc.length;i++)acc[i].addEventListener("mousedown",(function(){this.classList.toggle("active");var e=this.nextElementSibling;e.style.maxHeight?e.style.maxHeight=null:e.style.maxHeight=e.scrollHeight+"px"}));
...
var resetAllBtn=document.getElementById("reset-all-filter-btn"),allFiltersInputs=resetAllBtn.parentNode.getElementsByTagName("input"),changesCounter=[],allCheckbox=[],allSearches=[],notEmptyInputs=[];for(let e=0;e<allFiltersInputs.length;e++)"checkbox"==allFiltersInputs[e].type&&(allCheckbox.push(allFiltersInputs[e]),allFiltersInputs[e].addEventListener("change",e=>{if(1==e.target.checked){if(changesCounter.push(e.target),changesCounter.length>0){let t=e.target.parentNode.parentNode.parentNode.getElementsByClassName("reset-filter-btn");for(let e=0;e<t.length;e++)t[e].classList.add("active");resetAllBtn.classList.add("active")}}else if(0==(changesCounter=changesCounter.filter(t=>t!==e.target)).length&&0==notEmptyInputs.length){let t=e.target.parentNode.parentNode.parentNode.getElementsByClassName("reset-filter-btn");for(let e=0;e<t.length;e++)t[e].classList.remove("active");resetAllBtn.classList.remove("active")}})),"text"==allFiltersInputs[e].type&&(allSearches.push(allFiltersInputs[e]),allFiltersInputs[e].addEventListener("input",e=>{if(console.log(changesCounter,allSearches,notEmptyInputs),allSearches.forEach(e=>{""!==e.value?notEmptyInputs.push(e):notEmptyInputs=notEmptyInputs.filter(t=>t!==e)}),0==notEmptyInputs.length&&0==changesCounter.length){let t=e.target.parentNode.parentNode.parentNode.getElementsByClassName("reset-filter-btn");for(let e=0;e<t.length;e++)t[e].classList.remove("active");resetAllBtn.classList.remove("active")}if(notEmptyInputs.length>0){let t=e.target.parentNode.parentNode.parentNode.getElementsByClassName("reset-filter-btn");for(let e=0;e<t.length;e++)t[e].classList.add("active");resetAllBtn.classList.add("active")}}));function resetAll(e){for(let t=0;t<allFiltersInputs.length;t++)"checkbox"==allFiltersInputs[t].type&&1==allFiltersInputs[t].checked&&(allFiltersInputs[t].checked=!1),"text"==allFiltersInputs[t].type&&(""!==allFiltersInputs[t].value&&(allFiltersInputs[t].value=""),filter(e,allFiltersInputs[t]));changesCounter=[],notEmptyInputs=[],resetAllBtn.classList.remove("active");let t=document.getElementsByClassName("reset-filter-btn");for(let e=0;e<t.length;e++)t[e].classList.remove("active")}
function reset(e){for(var t=e.target.parentNode,n=t.getElementsByTagName("input"),s=0;s<n.length;s++)"checkbox"==n[s].type&&(n[s].checked=!1);var l=t.getElementsByClassName("filter-search-input__input");if(0!==l.length)for(s=0;s<l.length;s++)l[s].value="",filter(e);e.target.classList.remove("active"),t.getElementsByClassName("filter-search-input__input"),notEmptyInputs=notEmptyInputs.filter(e=>e!==l[0]);for(let e=0;e<n.length;e++)"checkbox"==n[e].type&&(changesCounter=changesCounter.filter(t=>t!==n[e]));0==notEmptyInputs.length&&0==changesCounter.length&&resetAllBtn.classList.remove("active")}
