...
var i,acc=document.getElementsByClassName("catalog-filters");for(i=0;i<acc.length;i++)acc[i].addEventListener("click",(function(){this.classList.toggle("active");var t=this.nextElementSibling;t.style.maxHeight?t.style.maxHeight=null:t.style.maxHeight=t.scrollHeight+"px"}));


function filter(e){var t,a,n,l;for(a=(t=e.target).value.toUpperCase(),n=t.parentNode.parentNode.getElementsByTagName("label"),l=0;l<n.length;l++)txtValue=n[l].textContent||n[l].innerText,txtValue.toUpperCase().indexOf(a)>-1?n[l].style.display="":n[l].style.display="none"}
var resetAllBtn=document.getElementById("reset-all-filter-btn"),allFiltersInputs=resetAllBtn.parentNode.getElementsByTagName("input");for(let e=0;e<allFiltersInputs.length;e++)allFiltersInputs[e].addEventListener("input",t=>{resetAllBtn.classList.add("active"),"checkbox"==uncheck[e].type&&(uncheck[e].checked=!1)});
function reset(e){for(var t=e.target.parentNode,n=t.getElementsByTagName("input"),a=0;a<n.length;a++)"checkbox"==n[a].type&&(n[a].checked=!1);var r=t.getElementsByClassName("filter-search-input__input");if(0!==r.length)for(a=0;a<r.length;a++)r[a].value="",filter(e)}
